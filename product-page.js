  ...
  Papa.parse(csvUrl, {
    ...
    complete: function(res) {
      ...
      const product = products.find(p =>
        p.id.toLowerCase() === productId || slugify(p.name) === productId
      );
      ...
      if (product.discount) {
        if (product.discount.includes("%")) finalPrice = product.price * (1 - parseFloat(product.discount)/100);
        else finalPrice = product.price - parseFloat(product.discount);
      }

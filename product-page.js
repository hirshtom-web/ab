...
// Mapping for physical sizes to price
const sizePriceMap = {
  "5x8": 30,
  "8x11": 36,
  "11x15": 44,
...
// Get selected type and size
function getSelectedProduct() {
  const type = artTypeSelect.value;
  let size = '';
  let price = 6.98; // default digital price

  if(type === 'physical') {
    size = artSizeSelect.value;
    price = sizePriceMap[size];
  }
...
buyNowBtn.addEventListener('click', () => {
  ...
  alert(`Buying ${product.type} ${product.size ? '- ' + product.size : ''} for $${product.price}`);

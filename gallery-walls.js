function initBundlesPage() {
 ...
 function updateGridImages() {
  ...
  cards.forEach(card => {
    ...
    // ✅ ALWAYS pick lifestyle image (last one in CSV)
    const lifestyleImgSrc = imgList[imgList.length - 1];
    const url = lifestyleImgSrc.startsWith("http")
      ? lifestyleImgSrc
      : "https://static.wixstatic.com/media/" + lifestyleImgSrc;

    // show lifestyle
    lifestyleImg.src = url;
    lifestyleImg.style.display = "block";

    // hide artwork but keep it in DOM
